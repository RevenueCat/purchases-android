...
4. The rest will be performed automatically by CircleCI, including merging the release PR. If the automation fails, you can revert to manually calling `bundle exec fastlane android deploy`.
5. Merge the bump to the next snapshot version PR right after.
...
1. When the PR is approved, **DO NOT MERGE IT**. The `wait-for-pr-approval` job is approved automatically once the PR is approved, and CircleCI tags the release when tests pass. If there is no such job because the release is older than when we introduced it, manually tag the last commit in `release/3.9.1` with `3.9.1`.
