...
android {
    namespace = "com.revenuecat.checkpointssample"
    compileSdk {
        version = release(36)
    }
    ...
    defaultConfig {
        ...
        // Defaults to the checkpoint tester's key (and then the paywall tester's) since all three apps
        // share the same applicationId.
        buildConfigField(
            "String",
            "CHECKPOINTS_SAMPLE_API_KEY",
            "\"${resolveProperty(
                "CHECKPOINTS_SAMPLE_API_KEY",
            ).ifEmpty { resolveProperty("CHECKPOINT_TESTER_API_KEY") }
