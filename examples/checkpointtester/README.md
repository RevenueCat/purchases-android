...
Each screen represents a way an app can react to the `FlowResult?` its `checkpoint()` callback receives,
which is where the differences between "hard paywall", "soft paywall" and "onboarding" actually live: the SDK
reports what the user obtained (or null when nothing was presented), and the app decides whether that gates
anything.
...
An identifier the dashboard doesn't know about, or a configured checkpoint whose rules don't match, presents
nothing: the callback receives null and the SDK logs say why.
...
**Custom checkpoint** — a text field for any identifier, plus the raw `FlowResult?` it produced. Nothing is
...
There is deliberately **no shared helper** wrapping the checkpoints API beyond a one-line `summary()` for display.
Every `ViewModel` calls `Purchases.sharedInstance.checkpoint(...)` itself and decides what a null result or an
empty grant list means for its use case, so each file shows the whole call in one place — including the
`CheckpointParams` construction. The callbacks look similar across screens on purpose: the branches resolve to
different behavior in each use case, and that difference is the thing worth reading.
