...
/**
 * The dummy custom paywall behind [DummyPaywallPresenter]: the offering's packages as plain buy
 * buttons plus dismiss and continue buttons. A cancelled purchase keeps the dialog open; any other purchase error is
 * shown inline so the user can retry or dismiss. It reports how the user left it (purchased, closed through the
 * dismiss button, continued without buying, or backed out through system back or a tap outside); the SDK works
 * out what the user obtained.
 */
@OptIn(InternalRevenueCatAPI::class)
@Composable
fun DummyPaywallDialog(
    request: DummyPaywallPresenter.Request,
    ...
    AlertDialog(
        onDismissRequest = { if (!purchasing) finish(PaywallPresenter.Completion.Result.NavigatedBack) },
        modifier = modifier,
        ...
        confirmButton = {
            TextButton(
                onClick = { finish(PaywallPresenter.Completion.Result.ContinuedWithoutPurchasing) },
                enabled = !purchasing,
            ) {
        ...
        dismissButton = {
            TextButton(onClick = { finish(PaywallPresenter.Completion.Result.Closed) }, enabled = !purchasing) {
                Text(text = "Dismiss")
            }
        },
    )
...
private fun PaywallContent(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Text(
            text = "Presented by the app's PaywallPresenter for offering " +
                "\"${request.params.offering.identifier}\".",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        request.params.offering.availablePackages.forEach { packageToPurchase ->
            Button(
