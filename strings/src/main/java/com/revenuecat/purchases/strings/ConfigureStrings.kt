...
object ConfigureStrings {
    ...
    const val INVALID_CREDENTIALS_ERROR = Emojis.ROBOT + Emojis.ERROR + " There was a credentials issue. " +
            "Check the underlying error for more details More info here: http://errors.rev.cat/play-service-credentials"
    const val LISTENER_SET = Emojis.INFO + " Listener set"
    const val NO_SHARED_SINGLETON = Emojis.WARNING + " There is no singleton instance. " +
