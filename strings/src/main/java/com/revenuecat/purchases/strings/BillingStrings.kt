...
object BillingStrings {
    const val BILLING_CLIENT_ENDED = "Ending connection for %s"
    const val BILLING_CLIENT_ERROR = "Billing Service Setup finished with error code: %s"
    const val BILLING_CLIENT_NULL = "Billing is disconnected and purchase methods won't work. Stacktrace: %s"
    const val BILLING_CLIENT_STARTED = "Starting connection for %s"
    ...
    const val BILLING_SETUP_FINISHED = "Billing Service Setup finished for %s"
    ...
    const val BILLING_WRAPPER_UPDATED = "BillingWrapper purchases updated: %s"
}
