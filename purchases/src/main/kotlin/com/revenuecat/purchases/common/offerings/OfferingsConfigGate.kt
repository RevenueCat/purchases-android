 ...
 *
 * In production this is backed by `RemoteConfigManager` (it ensures a config sync has happened and awaits the
 * `workflows` topic before signalling ready). `OfferingsManager` stays ignorant of which topics matter and of
 * coroutines: it just calls [awaitReady] and delivers in [onReady]. When no gate is wired, offerings deliver
 * immediately.
 */
