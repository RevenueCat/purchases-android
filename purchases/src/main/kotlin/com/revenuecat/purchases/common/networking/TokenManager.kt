...
internal class TokenManager(
     ...
     */
    data class RefreshedTokens(val accessToken: String, val refreshToken: String, val idToken: String)
    ...
    private val pendingRefreshes = mutableMapOf<String, MutableList<(RefreshedTokens?) -> Unit>>()
    ...
    fun tokenRefreshRequest(
        ...
        reportTokenUpdate: (RefreshedTokens?) -> Unit,
    ): TokenRefreshRequest? {
     ...
     * [RefreshedTokens], or `null` for a failed refresh -- replaying it to every `reportTokenUpdate` callback
     ...
     */
    fun handleTokenRefreshResponse(appUserID: String, tokens: RefreshedTokens?) {
        if (tokens != null) {
    ...
    @Synchronized
    private fun registerRefreshWaiter(appUserID: String, callback: (RefreshedTokens?) -> Unit): Boolean {
    ...
    @Synchronized
    private fun resolveRefreshWaiters(appUserID: String): List<(RefreshedTokens?) -> Unit> =
