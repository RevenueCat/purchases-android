...
internal object IndexOfOperator {
     ...
     * occurrence, or `-1` when there is none. The haystack comes first, like
     * every other `rc.` operator, even though `in` takes the needle first.
     *
     ...
     *
     * Absence returns `-1` rather than throwing: a value that is not there is
     * an ordinary answer, not a lowering bug, and `-1` keeps the result usable
     * in arithmetic. A haystack that is neither array nor string, or a
