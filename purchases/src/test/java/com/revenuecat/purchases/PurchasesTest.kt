...
class PurchasesTest {
    ...
    fun `given a successful aliasing, success handler is called`() {
        ...
        every {
            mockCache.setCustomerInfoCacheTimestampToNow("new_id")
        } just Runs
    ...
    fun `given an unsuccessful aliasing, onError handler is called`() {
        ...
        val mockReceiveCustomerInfoListener = mockk<ReceiveCustomerInfoListener>(relaxed = true)
        purchases.createAlias(
            "new_id",
            mockReceiveCustomerInfoListener
        )
        verify {
            mockReceiveCustomerInfoListener.onError(eq(purchasesError))
        }
    }
    ...
    fun `when creating alias, caches are updated`() {
        ...
        verify(exactly = 1) {
            mockBackend.getCustomerInfo("new_id", false, any(), any())
        }
        verify(exactly = 1) {
    ...
    fun `when identifying, user is identified and caches updated`() {
        ...
        verify(exactly = 1) {
            mockBackend.getCustomerInfo("new_id", any(), any(), any())
        }
        verify(exactly = 1) {
    ...
    fun `when resetting, caches are updated`() {
        ...
        verify(exactly = 1) {
            mockCache.setCustomerInfoCacheTimestampToNow("fakeUserID")
        }
        verify(exactly = 1) {
            mockCache.setOfferingsCacheTimestampToNow()
        }
        verify(exactly = 1) {
            mockBackend.getCustomerInfo("fakeUserID", any(), any(), any())
        }
        verify(exactly = 1) {
    ...
    fun `don't identify if the new app user id is the same`() {
        ...
        verify(exactly = 1) {
            mockCache.getCachedCustomerInfo(appUserId)
        }
        assertThat(receivedInfo).isEqualTo(mockInfo)
    }
