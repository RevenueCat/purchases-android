...
public interface PaywallWebViewController {
     ...
     * with SDK-managed variables (such as `locale` and `color_scheme`) and the paywall's custom
     * variables; use this to provide additional values, typically nested under the `custom` key.
     *
    ...
    /**
     * Sends an arbitrary message of the given [type] into the web view targeting [componentId]. The
     * fields in [message] are merged into the flat message envelope. Use [postVariables] for the common
     * `rc:variables` case.
     */
    public fun postMessage(
        componentId: String,
        type: String,
        message: Map<String, PaywallWebViewValue>,
    )
}
