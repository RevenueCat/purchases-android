...
class IdentityManager(
    ...
    fun configure(appUserID: String?) {
        val appUserIDToUse = appUserID
            ...
            ?: generateRandomID()
        log(LogIntent.USER, IdentityStrings.IDENTIFY_APP_USER_ID.format(appUserIDToUse))
    ...
    fun identify(
        ...
        if (currentUserIsAnonymous()) {
            log(LogIntent.USER, IdentityStrings.IDENTIFY_ANON_ID.format(appUserID))
    ...
    fun createAlias(
        ...
        log(LogIntent.USER, IdentityStrings.CREATE_ALIAS.format(currentAppUserID, newAppUserID))
        backend.createAlias(
            currentAppUserID,
            newAppUserID,
            {
                synchronized(this@IdentityManager) {
                    log(LogIntent.USER, IdentityStrings.CREATE_ALIAS_SUCCESS)
                    deviceCache.clearCachesForAppUserID(currentAppUserID)
                    subscriberAttributesCache.clearSubscriberAttributesIfSyncedForSubscriber(currentAppUserID)
    ...
    private fun generateRandomID(): String {
        return "\$RCAnonymousID:" + UUID.randomUUID().toString().toLowerCase(Locale.ROOT).replace("-", "")
            .also {
                log(LogIntent.USER, IdentityStrings.NEW_ANON_ID)
            }
    }
}
